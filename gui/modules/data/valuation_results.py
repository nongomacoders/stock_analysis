...
async def insert_valuation_result(connection, result):
    ...
    await connection.execute("""
        INSERT INTO deterministic_valuations
          (valuation_id, ticker, report_version_id, valuation_engine_version,
           generated_at, valuation_date, status, input_ids, calculation_inputs, preflight, result, published_at)
        VALUES ($1::uuid,$2,$3::uuid,$4,$5,$6,$7,$8::uuid[],$9::jsonb,$10::jsonb,$11::jsonb,
                CASE WHEN $7 IN ('PASS','PASS_WITH_WARNINGS') THEN NOW() ELSE NULL END)
    """, result.valuation_id, result.ticker, result.report_version_id,
        ...
        json_text(result.preflight), json_text(result.model_dump(mode="json")))
