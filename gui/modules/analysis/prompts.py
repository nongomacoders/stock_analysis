...
def build_research_prompt(deep_research_str=None):
    ...
    if deep_research_str:
        base_context = (
            "--- UNVERIFIED HISTORICAL CONTEXT (DEEP RESEARCH) ---\n"
            f"{deep_research_str}\n"
...
SOURCES:
1. Deep Research Report (unverified historical context).

{HISTORICAL_CONTEXT}
This task summarises the supplied report, not a new valuation. Preserve its
provenance caveats; do not present unresolved assumptions as verified facts.
...
def build_spot_price_prompt(
    ...
    commodity_avgs: list[dict] | None,
    fx_avgs: list[dict] | None,
):
    """Build a prompt to ask the AI for a "share price at spot" estimate.

    - commodity_avgs: metadata-rich price records
    - fx_avgs: metadata-rich FX records
    ...
    parts.append(format_market_context(commodity_avgs, fx_avgs) or "Commodity/FX averages: unavailable")
    parts.append(HISTORICAL_CONTEXT)

    parts.append("\n--- UNVERIFIED HISTORICAL CONTEXT ---")
    parts.append(deep_research or "<none>")
    parts.append("--- END HISTORICAL CONTEXT ---\n")
